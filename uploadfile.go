...
func upload(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM file_tb ORDER BY id DESC")
	if err != nil {
		panic(err.Error())
	}
	upld := upfile{}
...
func uploadFiles(w http.ResponseWriter, r *http.Request) {
	...
	//get the *fileheaders
	fil := formdata.File["files"] // filenames
	...
	for i := range fil {
		...
		insForm, err := db.Prepare("INSERT INTO file_tb(fname, fsize, ftype, path) VALUES(?,?,?,?)")
		if err != nil {
			panic(err.Error())
		} else {
